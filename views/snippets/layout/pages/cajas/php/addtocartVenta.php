...
if($quantityProduct<=0){
    print "<script>window.location='". URL . "cajas?caja=ventas&producto=". $nameProduct . "&estado=agotado';</script>";
}
else{
...
$sql = $con->query("SELECT * FROM `bills` WHERE users_idusers = $SESSION AND typeBill=1 AND stateBill=2");
$array = mysqli_fetch_array($sql);
$row1 = mysqli_num_rows($sql);
if($row1==0){
    $sql = $con->query("SELECT * FROM `bills` WHERE typeBill=1");
    $array = mysqli_fetch_array($sql);
    $row = mysqli_num_rows($sql);
    $numeroFactura = $row+1;
    $dateTime = date("Y-m-d");

    $sql = $con->query("INSERT INTO `bills` (`numeroFactura`, `users_idusers`, `cash_idcash`, `idCliente`, `cliente`, `typeBill`, `dateRegister`, `dateUpdate`, `total`, `impuesto`, `pago`, `saldo`, `stateBill`, `userUpdate`) VALUES ('$numeroFactura', '$SESSION', '1', '', '', '1', '$dateTime', '$dateTime', '0', '$iva', '0', '0', '2', '$SESSION');");
    ...
    $sqlUser = $con->query("SELECT * FROM `users` INNER JOIN userdetails ON idusers=users_idusers WHERE idusers=$SESSION");
