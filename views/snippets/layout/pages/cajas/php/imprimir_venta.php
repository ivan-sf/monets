...
$printer->setJustification(Printer::JUSTIFY_CENTER);
    $fecha = $_GET['fecha'];
	$array= $con->query("SELECT * FROM bills INNER JOIN billdetails ON idbills=bills_idbills WHERE bills.dateRegister='$fecha' AND typeBill=1 AND billdetails.stateBillDetail=1");
	$data = mysqli_fetch_array($array);
...
print "<script>window.location='".  URL ."facturas/detalles&id=0&dia=venta?fecha=$fecha';</script>";
